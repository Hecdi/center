...
const mergeFreightDB = (taskList) => {
	...
	each(taskList, (item, index) => {
		if (!areas[item.mainTaskId]) {
			...
			areas['mainStaffId'] = item.mainStaffId;
			areas['mainStaffName'] = item.mainStaffName;
	...
	each(data, (item) => {
		if(!result[item.mainStaffId]){
			...
			result[item.mainStaffId]['taskList'] = item.taskList;
			...
			if(!result[item.mainStaffId]['taskList']){
				...
				result[item.mainStaffId]['taskList'] = [...result[item.mainStaffId]['taskList'], ...item.taskList];
			};
		}
	});
	// data = filter(data, (item) => {
