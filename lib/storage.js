...
const taskListFilter = (filterOpt) =>{
	...
	if(opt.searchPersonKey){
		...
		let result = taskDB.chain().find(opt).compoundsort(['scheduleTime']).data();
		return result;
	}
}
...
const TableListFilter = (filterOpt) =>{
	...
	}else
	{
		...
		let result2 = homeTableDB.chain().find(opt).compoundsort(['scheduleTime']).offset(offsetSize).limit(pageSize).data();
		let result = {total:result1,data:result2};
		return result;
	}
}
