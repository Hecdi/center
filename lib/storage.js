...
const TableListFilter = (filterOpt) =>{
	...
	if(opt.searchPersonKey){
		...
		let result1=0;
		each(['flightNo','gate','seat','aircraftNumber','aircraftType','correlationFlightNo','areaName','taskBindingShiftName'],item=>{	
			// result1 = new Set([...result1,... new Set(homeTableDB.find({[item]:{'$regex':key}})).length]);
			result1 = result1 + homeTableDB.find({[item]:{'$regex':key}}).length;
			result2 = new Set([...result2,... new Set(homeTableDB.chain().find({[item]:{'$regex':key}}).offset(offsetSize).limit(opt.pageSize).data())]);
		});
		let result = {total:result1,data:[...result2]}
		return result;
	}else
