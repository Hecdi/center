...
import { has, extend, get, groupBy, each, omit, flatMap, flow, filter, cloneDeep } from 'lodash';

var DB = new loki('ctu.db', { adapter: 'memory' });


export var areaDB = DB.addCollection('area', {
	unique: ['areaId'],
...
// });
export const saveToAreaDB = (rows) => {
	areaDB.clear();
	return Promise.map(rows, row => {
		areaDB.insert(row);
	}).then(() =>{
		let result = areaDB.find();
...
export const getSearchPersons = (key) => {
	let result = cloneDeep(areaDB.find());
	if(!key){
		return Promise.resolve(result);
	}
	let a = flow([
