...
export const saveHomeTableDB = (data) => {
	...
	return Promise.resolve(homeTableDB.find());
}
...
const TableListFilter = (filterOpt) =>{
	...
	// if(opt.searchPersonKey){
	// 	let key = opt.searchPersonKey;
	// 	let offsetSize = opt.currentPage * opt.pageSize;
	// 	let result1 = homeTableDB.find({taskBindingShiftNames:{'$regex':key}}).length;
	// 	let result2 = homeTableDB.chain().find({taskBindingShiftNames:{'$regex':key}}).offset(offsetSize).limit(opt.pageSize).data();
	// 	// homeTableDB.chain().offset(offsetSize).limit(pageSize).data()
	...
	// 	let result = new Set();
	// 	each(['flightNo','gate','seat','aircraftNumber','aircraftType','correlationFlightNo','areaName','taskBindingShiftName'],item=>{	
	// 		result = new Set([...result,... new Set(homeTableDB.find({[item]:{'$regex':key}}))]);
	// 	});
	// 	return [...result];
	// }else
	// {
		...
		// let result1 = homeTableDB.find({taskStatus:opt.taskStatus}).length;
		// let result2 = homeTableDB.chain().find({taskStatus:opt.taskStatus}).offset(offsetSize).limit(opt.pageSize).data();
		delete opt.pageSize;
		delete opt.currentPage;
		let result1 = homeTableDB.find(opt).length;
		let result2 = homeTableDB.chain().find(opt).offset(offsetSize).limit(pageSize).data();
		// let result = taskDB.find(opt);
		let result = {total:result1,data:result2};
		debugger;
		return result;
...
export const getHomeTableFromDB = (value)=>{
	...
	// let offsetSize = pageSize * currentPage;
	// let pageNationResult =Promise.resolve( {total:homeTableDB.find().length,data:homeTableDB.chain().offset(offsetSize).limit(pageSize).data()});
	// return Promise.resolve(pageNationResult.taskListFilter(opt))
	// return Promise.resolve({total:homeTableDB.find().length,data:homeTableDB.chain().offset(offsetSize).limit(pageSize).data()});
}
