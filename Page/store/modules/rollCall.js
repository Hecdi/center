...
import { mapKeys, extend, forEach, get, set, assignIn, flow, filter, pick, map, every} from 'lodash';
...
const mutations = {
	...
	addPersonToTeam(state, { teamId, person}){
		forEach(state.team, item => {
			if(item.squadId == teamId && every(item.organization, i => (i.staffId != person.staffId))){
				item.organization.push(person);
			}
		})
	},
...
const getters = {
	...
	getCommitData: (state, getters, rootState) => (code) => {
		...
		let staffStatePropitys = ['workerId', 'deptId', 'nonArrivalReason', 'staffName', 'staffId', 'workerName', 'groupLeader', 'leaveStartTime', 'leaveEndTime', 'squadId', 'squadName'];
