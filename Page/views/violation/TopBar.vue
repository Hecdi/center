...
v-model="inputSearch" class="seach-input"
/>
...
/>
      ...
      <div class="dialog">
				<el-dialog title="单位管理" :visible.sync="dialogVisible" 
...
:model="form" label-width="80px">
						<el-form-item label="单位编号"> <el-input v-model="form.name" /> </el-form-item>
						<el-form-item label="单位编号"> <el-input v-model="form.name" /> </el-form-item>
						<el-form-item label="单位编号"> <el-input v-model="form.name" /> </el-form-item>
          </el-form>
					<div class="company-list">
						<div class="name" v-for="item in companys" 
:key="item.name"
>
							<span v-if="!item.isEdit" v-on:click="handEdit(item);"
> {{ item.name }} </span>
							<div v-else class="company-info"
>
								<p>
									{{ item.name }} <span> <i class="el-icon-edit" /> <i class="el-icon-close" v-on:click="closeEdit(item);"
/> </span>
                </p>
								<p>{{ item.code }}</p>
								<p>{{ item.number }}</p>
...
>提交</el-button>
			...
			<div><wait :is="currentView" keep-alive 
:status-value="currentView"
/></div>
		</el-row>
	</div>
</template>
