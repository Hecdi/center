							...
							<el-container>
								<el-aside width="95%">
									<el-row :gutter="10" class="first-row">
										...
										<el-col :span="4" >
											{{task.displayTaskStatus}} {{`${task.delay == true?'D':''}`}}
										</el-col>
									</el-row>
									<el-row :gutter="10" class="second-row">
										<el-col :span="24">
											<label class="flightNo">{{ `${task.flightNo}` }}</label>
											{{ `(${task.correlationFlightNo})|` }} <label class="aircraftNumber">{{ task.aircraftNumber }}</label> |<label v-for="(route, index) in task.airRoute" :key="route">
												<i v-if="index != 0" class="el-icon-caret-right"/>{{ route }}</label>
										</el-col>
									</el-row>
